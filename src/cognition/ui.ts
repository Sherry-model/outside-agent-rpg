...
interface Host {
  content: Content; state: State; turnOffset: number; episodeTitle?: string; returnLabel?: string;
...
export function mount(host?: Host) {
  ...
  function terminal() {
    ...
    if (result) {
      ...
      controls = `<section class="ending-block"><p class="eyebrow">这一段经历暂时结束</p><p class="ending-summary">${esc(node.title)}</p><button class="primary" data-view="memory">看看留下了什么</button> ${host ? `<button class="primary" data-act="return-main">${esc(host.returnLabel ?? '回到正式入口')} →</button>` : '<button class="text-button" data-act="restart">另一个午后 ↻</button>'}</section>`;
    } else {
    ...
    return `<div class="event-meta"><span>${esc(node.period ?? '一个普通的午后')}</span><span>${esc(host?.episodeTitle ?? '历史独立片段')}</span></div><div class="event-heading"><div><p class="eyebrow">SOME THINGS CAN WAIT</p><h1 id="event-title" tabindex="-1">${esc(node.title)}</h1></div></div><figure class="scene"><canvas id="scene" aria-label="当前场景的抽象像素画面" role="img"></canvas><figcaption>FIELD RECORDING / OUTSIDE</figcaption></figure><div class="prose">${node.text.map(t => `<p>${esc(t)}</p>`).join('')}</div>${currentNews.map(item => `<aside class="news-arrival"><p class="eyebrow">自行到来的消息 / ${esc(item.sourceLabel)}</p><p>${esc(item.text)}</p></aside>`).join('')}${controls}`;
  }
  function memories() {
  ...
  function action(name: string) {
    ...
    if (name === 'restart') dialog('<h2 id="modal-title">重新开始</h2><p class="modal-intro">会替换当前自动进度。可先导出记录；手动备份保留。</p><button class="primary" data-act="confirm-restart">开始新的经历</button> <button class="secondary" data-act="close">留在这里</button>');
  ...
  window.render_game_to_text = () => JSON.stringify({
    ...
    event: { id: state.instance.nodeId, title: currentNode(content, state).title, period: currentNode(content, state).period }, resources: state.instance.resources,
